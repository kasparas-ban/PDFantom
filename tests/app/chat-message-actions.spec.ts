...
test("shows when a message was sent and copies it from the tooltip", async ({ application }) => {
  ...
  await expect(reader.chatPanel.getByText("An answer", { exact: true })).toBeVisible()

  const hoverMessage = () =>
    expect
      .poll(async () => {
        await reader.chatPanel.getByText("Fix it for me.", { exact: true }).hover()
  ...
  await reader.chatPanel.getByText("An answer", { exact: true }).hover()
