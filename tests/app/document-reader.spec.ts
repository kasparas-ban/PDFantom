...
test("toggles the Chat panel", async ({ application }) => {
  ...
  await expect(reader.chatModelOption("GPT-5.4 Nano")).toBeHidden()
  await expect(reader.chatModelOption("GPT-5.4 Mini")).toBeVisible()
  await reader.chatModelFilterInput.press("ArrowDown")
  await expect(reader.chatModelOption("GPT-5.4 Mini")).toBeFocused()
  await reader.chatModelOption("GPT-5.4 Mini").press("Enter")
